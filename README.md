...
A production-ready, multi-stream (�?2) real-time video analytics pipeline with AI-powered object detection and tracking, featuring a modern web dashboard for monitoring and visualization.
...
- `./models` �?`/app/models` �?store Ultralytics `.pt` �?TensorRT `.engine` 模型�?

Environment overrides:

- `PIPELINE_CONFIG` �?pipeline service config path，默�?`/data/config/pipeline.yaml`�?
- `DASHBOARD_CONFIG` / `DASHBOARD_PORT` �?控制看板引用的配置与端口�?

> 若需在容器中使用 TensorRT 引擎，请确保宿主机具�?GPU 与匹配的 NVIDIA 驱动，并基于官方 TensorRT/CUDA 基础镜像构建，运行时添加 `--gpus all`�?
